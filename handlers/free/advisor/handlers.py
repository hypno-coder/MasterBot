...
from lexicon import (ANSWERS, AdvisorActionMenuButtons, AdvisorLexicon,
                     AdvisorMenuButtons)
...
@advisorHandlerRouter.callback_query(
    F.data.in_(
        [
            AdvisorMenuButtons.AskAnAdvisorAQuestion.name,
            AdvisorActionMenuButtons.AskTheAdvisorAgain.name,
        ]
    ),
    flags=flags,
)
async def ask_question(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await message.answer(text=AdvisorLexicon.ask_question)
...
async def answer_to_question(message: Message, bot: Bot) -> None:
    if message.text == None or message.from_user == None:
        return
    data = await bot.send_message(chat_id=message.chat.id, text=AdvisorLexicon.wait)
    ...
    await message.answer(
        text=AdvisorLexicon.another_question, reply_markup=advisor_action_menu_keyboard
    )
