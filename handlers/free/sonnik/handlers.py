...
from lexicon import (CommonLexicon, SonnikActionMenuButtons, SonnikLexicon,
                     SonnikMenuButtons)
...
@sonnikHandlerRouter.callback_query(
    F.data == SonnikMenuButtons.GetSonnik.name, flags=flags
)
@sonnikHandlerRouter.callback_query(
    F.data == SonnikActionMenuButtons.Repeat.name, flags=flags
)
async def start_sonnik_conv(
...
@sonnikHandlerRouter.message(
    lambda a: a.text and bool(re.match(r"^[А-Яа-я]+$", a.text)),
    FSMSonnik.sleeping_pattern,
)
async def process_image(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    await bot.edit_message_text(
        chat_id=chat_id, message_id=message_id, text=SonnikLexicon.download_message
    )
...
async def text_filter(message: Message, bot: Bot):
    ...
    data = await bot.send_message(
        chat_id=id, text=f"{message.text} {SonnikLexicon.wrong_message}"
    )
